...
-- Create table for club contacts
create table if not exists public.contactos_solicitudes (
  id int8 generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  club text not null,
  nombres text,
  presidente text,
  cargo text default 'Presidente',
  correo text
);

-- Enable RLS
alter table public.contactos_solicitudes enable row level security;
create policy "Enable all access for contactos_solicitudes" on public.contactos_solicitudes for all using (true) with check (true);
