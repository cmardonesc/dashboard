                ...
                filteredTests.map((test, idx) => (
                  <tr key={test.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-5 text-left sticky left-0 bg-white group-hover:bg-slate-50 border-r border-slate-50">
                      ...
                      {(() => {
                        const clubObj = Array.isArray(test.players.clubes) ? test.players.clubes[0] : test.players.clubes;
                        return (
                          <ClubBadge 
                            clubName={clubObj?.nombre || test.players.club || 'Sin Club'} 
                            idClub={test.players.id_club} 
                            clubs={clubs} 
                            logoSize="w-3 h-3" 
