    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div>
              ...
              <p className="text-white/50 text-[8px] md:text-[10px] font-bold uppercase tracking-[0.2em] md:tracking-[0.3em]">Gestión de Partidos y Rendimiento en Vivo</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
            {activeView !== 'list' && (
              <button 
                ...
                className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white/5 border border-white/10 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
              >
                Volver al Listado
              </button>
            )}
            {activeView === 'list' && (
              <button 
                ...
                className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-red-600 text-white rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg shadow-red-900/20"
              >
                Programar Partido
              </button>
            )}
      ...
      {activeView === 'list' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {matches.length === 0 ? (
            <div className="col-span-full bg-white rounded-[32px] md:rounded-[40px] p-10 md:p-20 text-center border border-slate-100 shadow-sm">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-200 text-2xl md:text-3xl">
                <i className="fa-solid fa-calendar-xmark"></i>
              </div>
              <h3 className="text-lg md:text-xl font-black text-slate-900 uppercase italic mb-2">No hay partidos programados</h3>
              <p className="text-slate-400 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">Comienza por programar el primer encuentro del microciclo</p>
            </div>
          ) : (
            matches.map(match => (
              <div key={match.id} className="bg-white rounded-[32px] md:rounded-[40px] border border-slate-100 shadow-sm hover:shadow-xl transition-all overflow-hidden group">
                <div className="bg-[#0b1220] p-6 md:p-8 text-white">
                  ...
                  <h4 className="text-xl md:text-2xl font-black italic uppercase tracking-tighter mb-1">vs {match.opponentName}</h4>
                  <p className="text-white/40 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">{match.type}</p>
                </div>
                <div className="p-6 md:p-8">
                  <div className="flex justify-between items-center mb-6 md:mb-8">
                    <div className="text-center">
                      <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Local</p>
                      <p className="text-2xl md:text-3xl font-black text-slate-900 italic">{match.scoreHome}</p>
                    </div>
                    <div className="text-slate-200 text-xl font-black italic">-</div>
                    <div className="text-center">
                      <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Rival</p>
                      <p className="text-2xl md:text-3xl font-black text-slate-900 italic">{match.scoreAway}</p>
                    </div>
                  </div>
                  {match.status === 'scheduled' && (
                    <button 
                      onClick={() => handleStartLive(match.id)}
                      className="w-full py-3 md:py-4 bg-red-600 text-white rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all"
                    >
                      Iniciar Partido
                    </button>
                  )}
                  {match.status === 'live' && (
                    <button 
                      ...
                      className="w-full py-3 md:py-4 bg-[#0b1220] text-white rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
                    >
                      Ver Dashboard Vivo
                    </button>
                  )}
      ...
      {activeView === 'create' && (
        <div className="bg-white rounded-[32px] md:rounded-[40px] border border-slate-100 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
          <div className="bg-[#0b1220] p-6 md:p-10 text-white">
            <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tighter">Configuración de Encuentro</h3>
            <p className="text-white/40 text-[9px] md:text-[10px] font-bold uppercase tracking-widest mt-1">Define la alineación y el equipo rival</p>
          </div>
          ...
          <div className="p-6 md:p-10 space-y-8 md:space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
              <div className="space-y-2">
                <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Nombre del Rival</label>
                <input 
                  type="text" 
                  value={newMatch.opponentName}
                  onChange={e => setNewMatch({...newMatch, opponentName: e.target.value})}
                  className="w-full bg-slate-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-3 md:py-4 text-xs font-bold outline-none focus:ring-2 focus:ring-red-500"
              ...
              <div className="space-y-2">
                <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Tipo de Partido</label>
                <select 
                  value={newMatch.type}
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Array(11).fill(null).map((_, i) => (
                    <select
                      ...
                      className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-[9px] md:text-[10px] font-bold outline-none focus:ring-2 focus:ring-red-500"
                    >
                  ...
                  {newMatch.opponentLineup?.map((p, i) => (
                    <div key={i} className="flex gap-2">
                      <input 
                        ...
                        className="w-12 md:w-16 bg-slate-50 border-none rounded-xl px-3 md:px-4 py-2 md:py-3 text-[9px] md:text-[10px] font-bold outline-none focus:ring-2 focus:ring-slate-900"
                      />
                      <input 
                        type="text" 
                        ...
                        className="flex-1 bg-slate-50 border-none rounded-xl px-3 md:px-4 py-2 md:py-3 text-[9px] md:text-[10px] font-bold outline-none focus:ring-2 focus:ring-slate-900"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="pt-8 md:pt-10 border-t border-slate-100 flex justify-end">
              <button 
                ...
                className="w-full sm:w-auto px-8 md:px-12 py-4 md:py-5 bg-[#0b1220] text-white rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest hover:bg-red-600 transition-all shadow-xl"
              >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
              <div className="text-center md:text-left">
                <div className="w-12 h-12 md:w-16 md:h-16 bg-red-600 rounded-xl md:rounded-2xl flex items-center justify-center mb-3 md:mb-4 mx-auto md:mx-0 shadow-lg shadow-red-900/40">
                  <span className="text-xl md:text-2xl font-black italic">CHL</span>
                </div>
                <h3 className="text-xl md:text-3xl font-black italic uppercase tracking-tighter">Chile Sub-20</h3>
              </div>

              <div className="flex items-center gap-6 md:gap-12">
                <span className="text-5xl md:text-8xl font-black italic tracking-tighter">{activeMatch.scoreHome}</span>
                ...
                <span className="text-5xl md:text-8xl font-black italic tracking-tighter">{activeMatch.scoreAway}</span>
              </div>
              ...
              <div className="text-center md:text-right">
                <div className="w-12 h-12 md:w-16 md:h-16 bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center mb-3 md:mb-4 mx-auto md:ml-auto shadow-lg">
                  <span className="text-xl md:text-2xl font-black italic">{activeMatch.opponentName.substring(0, 3).toUpperCase()}</span>
                </div>
                <h3 className="text-xl md:text-3xl font-black italic uppercase tracking-tighter">{activeMatch.opponentName}</h3>
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-8 space-y-6 md:space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Gol Chile</span>
                </button>
                <button 
                ...
                >
                  <i className="fa-solid fa-futbol text-2xl md:text-3xl"></i>
                  <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Gol Rival</span>
                </button>
              </div>

              {/* LINEUP & SUBS */}
              <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 border border-slate-100 shadow-sm">
                <h4 className="text-[10px] md:text-xs font-black text-slate-900 uppercase tracking-[0.2em] mb-6 md:mb-8 flex items-center gap-2">
                  <span className="w-1.5 h-4 bg-red-600 rounded-full"></span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                  {/* TITULARES */}
                  <div>
                    <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 md:mb-4">En Cancha</p>
                    <div className="space-y-2">
                      {activeMatch.starters.map(id => (
                        <div key={id} className="flex items-center justify-between p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100 group">
                          <span className="text-[9px] md:text-[10px] font-bold text-slate-700">{getPlayerName(id)}</span>
                          <button 
                            onClick={() => {
                            ...
                            className="text-blue-500 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity text-[8px] md:text-[9px] font-black uppercase tracking-widest"
                          >
                  ...
                  {/* RESERVAS */}
                  <div>
                    <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 md:mb-4">Banca</p>
                    <div className="space-y-2">
                      {activeMatch.reserves.map(id => (
                        <div key={id} className="p-3 md:p-4 bg-slate-50/50 rounded-xl md:rounded-2xl border border-slate-100">
                          <span className="text-[9px] md:text-[10px] font-bold text-slate-400 italic">{getPlayerName(id)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="lg:col-span-4 bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 border border-slate-100 shadow-sm">
              ...
              <div className="space-y-4 md:space-y-6">
                {activeMatch.events.length === 0 ? (
                  <p className="text-[9px] md:text-[10px] font-bold text-slate-300 italic text-center py-6 md:py-10">Esperando eventos...</p>
                ) : (
                  activeMatch.events.slice().reverse().map((event, i) => (
                    <div key={i} className="flex gap-3 md:gap-4 items-start">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 shrink-0">
                        {event.type === 'goal' && <i className="fa-solid fa-futbol text-emerald-500 text-sm md:text-base"></i>}
                        {event.type === 'sub' && <i className="fa-solid fa-right-left text-blue-500 text-sm md:text-base"></i>}
                      </div>
                      <div>
                        <p className="text-[9px] md:text-[10px] font-black text-slate-900 italic">{event.minute}' - {event.type.toUpperCase()}</p>
                        <p className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">{event.detail || (event.playerId ? getPlayerName(event.playerId) : `Rival N°${event.opponentPlayerNumber}`)}</p>
                      </div>
                    </div>
                  ))
                )}
